...
describe('adminUserPasswordUpdate', () => {
    ...
    test('AuthUserId is not a valid user', () => {
        clear();
        expect(adminUserPasswordUpdate(user.userId + 1, user.password, 'abcd1234')).toEqual({ error: expect.any(String) });
    });
    ...
    test('Old Password is not the correct old password', () => {
        clear();
        expect(adminUserPasswordUpdate(user.userId, user.password, user.password)).toEqual({ error: expect.any(String) });
    });

    test('New Password has already been used before by this user', () => {
        clear();
        adminUserPasswordUpdate(user.userId, user.password, 'abcd1234');
        expect(adminUserPasswordUpdate(user.userId, 'abcd1234', '1234abcd')).toEqual({ error: expect.any(String) });
    });

    test('invalid password length', () => {
        clear();
        expect(adminUserPasswordUpdate(user.userId, 'abcd1234', '')).toEqual({ error: expect.any(String)});
        expect(adminUserPasswordUpdate(user.userId, 'abcd1234', '1')).toEqual({ error: expect.any(String)});
        expect(adminUserPasswordUpdate(user.userId, 'abcd1234', '12')).toEqual({ error: expect.any(String)});
        expect(adminUserPasswordUpdate(user.userId, 'abcd1234', '123')).toEqual({ error: expect.any(String)});
        expect(adminUserPasswordUpdate(user.userId, 'abcd1234', '1234')).toEqual({ error: expect.any(String)});
        expect(adminUserPasswordUpdate(user.userId, 'abcd1234', '12345')).toEqual({ error: expect.any(String)});
        expect(adminUserPasswordUpdate(user.userId, 'abcd1234', '123456')).toEqual({ error: expect.any(String)});
        expect(adminUserPasswordUpdate(user.userId, 'abcd1234', '1234567')).toEqual({ error: expect.any(String)});
    });

    test('unsatisfactory password', () => {
        clear();
        expect(adminUserPasswordUpdate(user.userId, 'abcd1234', '12345678')).toEqual({ error: expect.any(String)});
        expect(adminUserPasswordUpdate(user.userId, 'abcd1234', 'abcdefgh')).toEqual({ error: expect.any(String)});
    });
