...
// adminQuizTransfer:
// Info: Create two user and only the first has a quiz.
...
describe('Testing PUT /v1/admin/quiz/{qizId}/transfer', () => {
  ...
  test.todo.each([
    {
      token: user2.token,
      email: "", // FIX: how to check if real email?
      quizId: quiz2.quizId
    }, 
    {
    ...
    {
      token: user.token, // FIX: END STATE??? 
      email: "second@unsw.edu.au",
      quizId: quiz.quizId
    } 
  ...
  test('Invalid Token', () => {
    ...
    // second user (Note: no quiz for second user)
  ...
  // Error: Valid token, wrong quizId.
